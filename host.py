...
def measure_and_log(dt_for_filename, time_to_measure):
    ...
    counter = 0 # used for file naming
    for ch in scope.channels[0:3]:
        if ch.enabled:
...
def create_absO_time(time_interval, times_logged):
    interval = float(time_interval)/1000
    return np.arange(0, time_interval*times_logged, interval)

def add_time_tailgate(filename_in, abso_time):
    filename_out = filename_in + "_abs_time.txt"
    abso_count = 0
    with open(filename_in, 'r') as istr:
        with open(filename_out, 'w') as ostr:
            for line in istr:
...
[sg.Frame(layout=[ 
    [sg.Text('Slope:'), sg.Combo(['positive', 'negative'], key='tri_slope'),
     sg.Text('Level'), sg.In(size=(8,1), default_text = 0, key='tri_level'), sg.Text('V'),
     sg.Text('Source Channel:'), sg.Combo([0,1,2,3], key='tri_source')],
    [sg.Checkbox('Continue After Trigger', default = True, key='tri_cont'),
     sg.Text('Modifier:'), sg.Combo(['auto', 'none'], key='tri_modifier')],
...
while True:
    ...
    if event == 'Start Logging':
        ...
        while True:
            ...
            if event == 'Stop Logging':
                ...
                for i in range(0, len(dt_for_filename)):
                    if dt_for_filename[i] is not ' ': # well its a way of what channels were activated
                        add_time_tailgate(dt_for_filename[i], abso_time)
            ...
            # times to log 
            elif is_greater_than_zero(values['times_to_log']) and int(values['times_to_log']) > 0:
